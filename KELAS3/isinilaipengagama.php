...
echo"<p align='center'>";
	...
	$querykd="SELECT * FROM kd_peng_agama"; $hasilkd=mysql_query($querykd) or die(mysql_error()); $banyakkd=mysql_num_rows($hasilkd);
	
	if($datas[4]==1)
	{
		$query="SELECT*FROM nilai_peng_agama_ganjil WHERE tahun_ajaran ='$datas[5]'"; $query3="SELECT * FROM bobot_penilaian_ap WHERE mapel='Agama'";
		$hasil=mysql_query($query) or die(mysql_error()); $hasil3=mysql_query($query3) or die(mysql_error()); $data3=mysql_fetch_array ($hasil3);
	}
	else if($datas[4]==2)
	{
		$query="SELECT*FROM nilai_peng_agama_genap WHERE tahun_ajaran ='$datas[5]'"; $query3="SELECT * FROM bobot_penilaian_ap WHERE mapel='Agama'";
		$hasil=mysql_query($query) or die(mysql_error()); $hasil3=mysql_query($query3) or die(mysql_error()); $data3=mysql_fetch_array ($hasil3);
	}
	include"atas.php";
...
{ 
	...
	echo"<form method='GET' action='isinilaipengagama.php'>";
	echo"<p align='center'>";
	echo"<table border=0>";
	echo"<tr>";
	echo"<td><b>Aspek</b></td><td>:</td><td><b>Pengetahuan</b></td>"; echo"</tr>"; echo"<tr>";
	echo"<td><b>Mata Pelajaran</b></td><td>:</td><td><b>Agama</b></td>";
	echo"</tr>"; 
	echo"</table>"; echo"</br>";
	...
	echo"<td colspan='$banyakkd'><p align='center'><b>Nilai Harian</b></p></td>";
	echo"<td colspan='$banyakkd'><p align='center'><b>PTS</b></p></td>";
	echo"<td colspan='$banyakkd'><p align='center'><b>PAS</b></p></td>";
	...
	{	$querykd="SELECT * FROM kd_peng_agama"; $hasilkd=mysql_query($querykd); /////BERMASALAH JIKA DIHILANGKAN!!!!
		while ($datakd = mysql_fetch_array ($hasilkd))
		{
	...
	while ($data=mysql_fetch_array ($hasil))
	{
	 ...
	 {	
		$aa = $a+1;
		echo"<td><input type='text' name='nhkd$aa"."[]' size='5' value='$himpunannilai1[$a]'></td>";
	 }
	 for($b=0; $b<$banyakkd; $b++)
	 {	
		$bb = $b+1;
		echo"<td><input type='text' name='ptskd$bb"."[]' size='5' value='$himpunannilai2[$b]'></td>";
	 }
	 for($c=0; $c<$banyakkd; $c++)
	 {	
		$cc = $c+1;
		echo"<td><input type='text' name='paskd$cc"."[]' size='5' value='$himpunannilai3[$c]'></td>";
	 }
...
{ $bnp=$_GET['bnp']; $buts=$_GET['buts']; $buas=$_GET['buas']; 
	$query4="UPDATE bobot_penilaian_ap SET bobot_np='$bnp',bobot_uts='$buts',bobot_uas='$buas' WHERE mapel='Agama'"; 
	$hasil4=mysql_query($query4) or die(mysql_error());
	
	echo"<form method='GET' action='isinilaipengagama.php'>"; echo"<p align='center'>";
	echo"<table border=0>";
	echo"<tr>";
	echo"<td><b>Aspek</b></td><td>:</td><td><b>Pengetahuan</b></td>"; echo"</tr>"; echo"<tr>";
	echo"<td><b>Mata Pelajaran</b></td><td>:</td><td><b>Agama</b></td>";
	...
	for($awal=0; $awal<3; $awal++)
	{	$querykd="SELECT * FROM kd_peng_agama"; $hasilkd=mysql_query($querykd); 
		while ($datakd = mysql_fetch_array ($hasilkd))
		{
			echo"<td><p align='center'><input type='hidden' name='kodekd[]' value='$datakd[1]'><b>$datakd[1]</b></p></td>";
		}
...
else if (isset($_GET['hapuskd']))
{
	$querys="SELECT * FROM data_gurukelas"; $hasils=mysql_query($querys) or die(mysql_error()); $datas=mysql_fetch_array ($hasils);
	$querykd="SELECT * FROM kd_peng_agama"; $hasilkd=mysql_query($querykd) or die(mysql_error()); $banyakkd=mysql_num_rows($hasilkd);
	$queryindekskd="SELECT * FROM kd_peng_agama"; $hasilindekskd=mysql_query($queryindekskd) or die(mysql_error()); $dataindekskd = mysql_fetch_array ($hasilindekskd);
	...
	if ($banyakkd > 3)
	{
		if($datas[4]==1)
		{
			$queryadd1 = "ALTER TABLE nilai_peng_agama_ganjil DROP COLUMN nh_kd$banyakkd "; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
			$queryadd2 = "ALTER TABLE nilai_peng_agama_ganjil DROP COLUMN pts_kd$banyakkd "; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
			$queryadd3 = "ALTER TABLE nilai_peng_agama_ganjil DROP COLUMN pas_kd$banyakkd "; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
			$querydisplay="SELECT * FROM nilai_peng_agama_ganjil WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
		}
		else if($datas[4]==2)
		{
			$queryadd1 = "ALTER TABLE nilai_peng_agama_genap DROP COLUMN nh_kd$banyakkd "; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
			$queryadd2 = "ALTER TABLE nilai_peng_agama_genap DROP COLUMN pts_kd$banyakkd "; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
			$queryadd3 = "ALTER TABLE nilai_peng_agama_genap DROP COLUMN pas_kd$banyakkd "; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
			$querydisplay="SELECT * FROM nilai_peng_agama_genap WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
		}
		$queryadd4 = "DELETE FROM kd_peng_agama WHERE nomor='$banyakkd'"; $hasiladd4=mysql_query($queryadd4) or die(mysql_error());
	}
	else
	{
		if($datas[4]==1)
...
{	$querys="SELECT * FROM data_gurukelas"; $hasils=mysql_query($querys) or die(mysql_error()); $datas=mysql_fetch_array ($hasils);
	...
	if($datas[4]==1)
	{
		$queryadd1 = "ALTER TABLE nilai_peng_agama_ganjil ADD COLUMN nh_kd$baru decimal(6,2) NOT NULL AFTER nh_kd$banyakkd"; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
		$queryadd2 = "ALTER TABLE nilai_peng_agama_ganjil ADD COLUMN pts_kd$baru decimal(6,2) NOT NULL AFTER pts_kd$banyakkd"; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
		$queryadd3 = "ALTER TABLE nilai_peng_agama_ganjil ADD COLUMN pas_kd$baru decimal(6,2) NOT NULL AFTER pas_kd$banyakkd"; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
		$querydisplay="SELECT * FROM nilai_peng_agama_ganjil WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
	}
	else if($datas[4]==2)
	{
		$queryadd1 = "ALTER TABLE nilai_peng_agama_genap ADD COLUMN nh_kd$baru decimal(6,2) NOT NULL AFTER nh_kd$banyakkd"; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
		$queryadd2 = "ALTER TABLE nilai_peng_agama_genap ADD COLUMN pts_kd$baru decimal(6,2) NOT NULL AFTER pts_kd$banyakkd"; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
		$queryadd3 = "ALTER TABLE nilai_peng_agama_genap ADD COLUMN pas_kd$baru decimal(6,2) NOT NULL AFTER pas_kd$banyakkd"; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
		$querydisplay="SELECT * FROM nilai_peng_agama_genap WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
	}
	
	$queryadd4 = "INSERT INTO kd_peng_agama (nomor) VALUES ('$baru')"; $hasiladd4=mysql_query($queryadd4) or die(mysql_error());
	$querykd="SELECT * FROM kd_peng_agama"; $hasilkd=mysql_query($querykd) or die(mysql_error()); $banyakkd=mysql_num_rows($hasilkd);
...
else if (isset($_GET['simpankd']))
	{
		...
		{	
			$querysimpankd="UPDATE kd_peng_agama SET kode_kd='$kkd[$k]', kd='$kd[$k]' WHERE nomor='$nomor[$k]'";
	...
	{ 	
		...
		if ($datas[4]!='')
	{
		$noabsen=$_GET['nabsen']; $nama=$_GET['nama']; $baris=$_GET['jumbaris']; 
		
		$pembagi = array(); $nhkd = array(); $ptskd = array(); $paskd = array(); $ratakd = array(); $pos1 = array(); $pos2 = array(); $pos3 = array(); $himpunankd = array(); 
		...
		for ($i=0; $i<$baris; $i++)
		{
			...
			{	
				...
				$pembagi[$h] = 0;
				$indekskd = $h + 1;
				$nhkd[$h]=$_GET['nhkd'.$indekskd]; $ptskd[$h]=$_GET['ptskd'.$indekskd]; $paskd[$h]=$_GET['paskd'.$indekskd];
				//var_dump($nhkd);
			
				if (($nhkd[$h][$i] != null) or ($nhkd[$h][$i] != '')){$pembagi[$h]++;}  //bisa pake <> !=
				if (($ptskd[$h][$i] != null) or ($nhkd[$h][$i] != '')){$pembagi[$h]++;}
				if (($paskd[$h][$i] != null) or ($nhkd[$h][$i] != '')){$pembagi[$h]++;}
				
				$pos1[$h]=strpos($nhkd[$h][$i],".");				//mencari posisi tanda koma dan menggantinya dengan titik 
				if ($pos1[$h] = -1){$nhkd[$h][$i] = str_replace(",",".",$nhkd[$h][$i]);} 
				$pos2[$h]=strpos($ptskd[$h][$i],".");
				if ($pos2[$h] = -1){$ptskd[$h][$i] = str_replace(",",".",$ptskd[$h][$i]);} 
				$pos3[$h]=strpos($paskd[$h][$i],".");
				if ($pos3[$h] = -1){$paskd[$h][$i] = str_replace(",",".",$paskd[$h][$i]);}  
				
				$ratakd[$h] = (2*($nhkd[$h][$i]) + $ptskd[$h][$i] + $paskd[$h][$i])/(($pembagi[$h]) + 1 );
				$totalratakd = $totalratakd + $ratakd[$h];
				$himpunanratakd[$h] = $ratakd[$h];
				$querykd="SELECT * FROM kd_peng_agama WHERE nomor='$nomor'"; $hasilkd=mysql_query($querykd); $datakd = mysql_fetch_array($hasilkd);
				$himpunankd[$h] = array ($ratakd[$h] , $datakd[2]);
				//echo $himpunankd[$h][0]." ".$himpunankd[$h][1]."</br>";
				
				if($datas[4]==1)
				{
					$querysimpan = "UPDATE nilai_peng_agama_ganjil SET nh_kd$indekskd ='".$nhkd[$h][$i]."', pts_kd$indekskd ='".$ptskd[$h][$i]."', pas_kd$indekskd ='".$paskd[$h][$i]."'
				...
				{
					$querysimpan = "UPDATE nilai_peng_agama_genap SET nh_kd$indekskd ='".$nhkd[$h][$i]."', pts_kd$indekskd ='".$ptskd[$h][$i]."', pas_kd$indekskd ='".$paskd[$h][$i]."'
								WHERE no_absen='".$noabsen[$i]."'";
					$hasilsimpan=mysql_query($querysimpan) or die(mysql_error());
				}
			}
			...
			if($datas[4]==1)
			{
				$querysimpan = "UPDATE nilai_peng_agama_ganjil SET  nilai_100 ='$nilaiakhir[$i]', nilai_4 ='$nilai4[$i]', predikat ='$predikat[$i]', 
				...
				$hasilsimpan=mysql_query($querysimpan) or die(mysql_error());
				$query="SELECT*FROM nilai_peng_agama_ganjil WHERE tahun_ajaran ='$datas[5]'"; $hasil=mysql_query($query) or die(mysql_error());
			}
			else if($datas[4]==2)
			{
				$querysimpan = "UPDATE nilai_peng_agama_genap SET nilai_100 ='$nilaiakhir[$i]', nilai_4 ='$nilai4[$i]', predikat ='$predikat[$i]',
								deskripsi_positif= '$deskripsipositif[$i]' , deskripsi_negatif='$deskripsinegatif[$i]', keterangan ='$keterangan[$i]' WHERE no_absen='$noabsen[$i]'";
				$hasilsimpan=mysql_query($querysimpan) or die(mysql_error());
				$query="SELECT*FROM nilai_peng_agama_genap WHERE tahun_ajaran ='$datas[5]'"; $hasil=mysql_query($query) or die(mysql_error());
			}
		}
	}
	...
	$querykd="SELECT * FROM kd_peng_agama"; $hasilkd=mysql_query($querykd) or die(mysql_error());
	$bariskd=0; $i = 1;
		while ($datakd = mysql_fetch_array ($hasilkd))
		{	
			echo"<tr>";
