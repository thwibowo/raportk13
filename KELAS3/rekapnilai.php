...
echo"<p align='center'>";
	...
	while(($data=mysql_fetch_array($hasil)) and ($datapagama=mysql_fetch_array ($hasilpagama)) and
			($datapmtk=mysql_fetch_array ($hasilpmtk)) and ($datapbi=mysql_fetch_array ($hasilpbi)) and ($datappkn=mysql_fetch_array ($hasilppkn)) and
			($datapsbp=mysql_fetch_array ($hasilpsbp)) and ($datapbd=mysql_fetch_array ($hasilpbd)) and ($datappjok=mysql_fetch_array ($hasilppjok))
			 and ($datakagama=mysql_fetch_array ($hasilkagama)) and
	...
	{		//echo $data[2];echo"</br>";
			...
			mysql_select_db("raportk13_4",$koneksi1) or die(mysql_error());
			$queryres="SELECT * FROM data_siswa WHERE no_induk='$data[2]'"; $hasilres=mysql_query($queryres) or die(mysql_error()); $hasilcari=mysql_num_rows($hasilres);
			//echo $hasilcari;
		if($hasilcari!=1)
		{
			echo"<tr>";
