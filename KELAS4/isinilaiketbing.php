...
echo"<p align='center'>";
	...
	$querykd="SELECT * FROM kd_ket_bing"; $hasilkd=mysql_query($querykd) or die(mysql_error()); $banyakkd=mysql_num_rows($hasilkd);
	$query3="SELECT * FROM bobot_penilaian_ak WHERE mapel='BING'"; $hasil3=mysql_query($query3) or die(mysql_error()); $data3=mysql_fetch_array ($hasil3);
	
	if($datas[4]==1)
	{
		$query="SELECT*FROM nilai_ket_bing_ganjil WHERE tahun_ajaran ='$datas[5]'";  $hasil=mysql_query($query) or die(mysql_error()); 
	}
	else if($datas[4]==2)
	{
		$query="SELECT*FROM nilai_ket_bing_genap WHERE tahun_ajaran ='$datas[5]'";  $hasil=mysql_query($query) or die(mysql_error()); 
	}
	include"atas.php";
if( (!isset($_GET['simpankd'])) && (!isset($_GET['simpannilai'])) && (!isset($_GET['simpanbobot']))  && (!isset($_GET['tambahkd'])) && (!isset($_GET['hapuskd'])))
{ 
	
	echo"<form method='GET' action='isinilaiketbing.php'>";
	echo"<p align='center'>";
	echo"<table border=0>";
	echo"<tr>";
	...
	{	$querykd="SELECT * FROM kd_ket_bing"; $hasilkd=mysql_query($querykd); /////BERMASALAH JIKA DIHILANGKAN!!!!
		while ($datakd = mysql_fetch_array ($hasilkd))
		{
...
{ $bnp=$_GET['bnp']; $buts=$_GET['buts']; $buas=$_GET['buas']; 
	$query4="UPDATE bobot_penilaian_ak SET bobot_ujk='$bnp',bobot_proj='$buts',bobot_port='$buas' WHERE mapel='BING'";  $hasil4=mysql_query($query4) or die(mysql_error());
	$query3="SELECT * FROM bobot_penilaian_ak WHERE mapel='BING'"; $hasil3=mysql_query($query3) or die(mysql_error()); $data3=mysql_fetch_array ($hasil3);
	
	
	echo"<form method='GET' action='isinilaiketbing.php'>"; echo"<p align='center'>";
	echo"<table border=0>";
	echo"<tr>";
	echo"<td><b>Aspek</b></td><td>:</td><td><b>Keterampilan</b></td>"; echo"</tr>"; echo"<tr>";
	echo"<td><b>Mata Pelajaran</b></td><td>:</td><td><b>Bahasa Inggris</b></td>";
	...
	for($awal=0; $awal<3; $awal++)
	{	$querykd="SELECT * FROM kd_ket_bing"; $hasilkd=mysql_query($querykd); 
		while ($datakd = mysql_fetch_array ($hasilkd))
		{
			echo"<td><p align='center'><input type='hidden' name='kodekd[]' value='$datakd[1]'><b>$datakd[1]</b></p></td>";
		}
...
else if (isset($_GET['hapuskd']))
{
	$querys="SELECT * FROM data_gurukelas"; $hasils=mysql_query($querys) or die(mysql_error()); $datas=mysql_fetch_array ($hasils);
	$querykd="SELECT * FROM kd_ket_bing"; $hasilkd=mysql_query($querykd) or die(mysql_error()); $banyakkd=mysql_num_rows($hasilkd);
	$queryindekskd="SELECT * FROM kd_ket_bing"; $hasilindekskd=mysql_query($queryindekskd) or die(mysql_error()); $dataindekskd = mysql_fetch_array ($hasilindekskd);
	...
	if ($banyakkd > 3)
	{
		if($datas[4]==1)
		{
			$queryadd1 = "ALTER TABLE nilai_ket_bing_ganjil DROP COLUMN praktek_kd$banyakkd "; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
			$queryadd2 = "ALTER TABLE nilai_ket_bing_ganjil DROP COLUMN produk_kd$banyakkd "; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
			$queryadd3 = "ALTER TABLE nilai_ket_bing_ganjil DROP COLUMN proyek_kd$banyakkd "; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
			$querydisplay="SELECT * FROM nilai_ket_bing_ganjil WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
		}
		else if($datas[4]==2)
		{
			$queryadd1 = "ALTER TABLE nilai_ket_bing_genap DROP COLUMN praktek_kd$banyakkd "; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
			$queryadd2 = "ALTER TABLE nilai_ket_bing_genap DROP COLUMN produk_kd$banyakkd "; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
			$queryadd3 = "ALTER TABLE nilai_ket_bing_genap DROP COLUMN proyek_kd$banyakkd "; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
			$querydisplay="SELECT * FROM nilai_ket_bing_genap WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
		}
		$queryadd4 = "DELETE FROM kd_ket_bing WHERE nomor='$banyakkd'"; $hasiladd4=mysql_query($queryadd4) or die(mysql_error());
	}
	else
	{
		if($datas[4]==1)
...
{	$querys="SELECT * FROM data_gurukelas"; $hasils=mysql_query($querys) or die(mysql_error()); $datas=mysql_fetch_array ($hasils);
	...
	if($datas[4]==1)
	{
		$queryadd1 = "ALTER TABLE nilai_ket_bing_ganjil ADD COLUMN praktek_kd$baru decimal(6,2) NOT NULL AFTER praktek_kd$banyakkd"; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
		$queryadd2 = "ALTER TABLE nilai_ket_bing_ganjil ADD COLUMN produk_kd$baru decimal(6,2) NOT NULL AFTER produk_kd$banyakkd"; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
		$queryadd3 = "ALTER TABLE nilai_ket_bing_ganjil ADD COLUMN proyek_kd$baru decimal(6,2) NOT NULL AFTER proyek_kd$banyakkd"; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
		$querydisplay="SELECT * FROM nilai_ket_bing_ganjil WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
	}
	else if($datas[4]==2)
	{
		$queryadd1 = "ALTER TABLE nilai_ket_bing_genap ADD COLUMN praktek_kd$baru decimal(6,2) NOT NULL AFTER praktek_kd$banyakkd"; $hasiladd1=mysql_query($queryadd1) or die(mysql_error()); 
		$queryadd2 = "ALTER TABLE nilai_ket_bing_genap ADD COLUMN produk_kd$baru decimal(6,2) NOT NULL AFTER produk_kd$banyakkd"; $hasiladd2=mysql_query($queryadd2) or die(mysql_error());
		$queryadd3 = "ALTER TABLE nilai_ket_bing_genap ADD COLUMN proyek_kd$baru decimal(6,2) NOT NULL AFTER proyek_kd$banyakkd"; $hasiladd3=mysql_query($queryadd3) or die(mysql_error());
		$querydisplay="SELECT * FROM nilai_ket_bing_genap WHERE tahun_ajaran='$datas[5]'"; $hasildisplay=mysql_query($querydisplay) or die(mysql_error());
	}
	
	$queryadd4 = "INSERT INTO kd_ket_bing (nomor) VALUES ('$baru')"; $hasiladd4=mysql_query($queryadd4) or die(mysql_error());
	$querykd="SELECT * FROM kd_ket_bing"; $hasilkd=mysql_query($querykd) or die(mysql_error()); $banyakkd=mysql_num_rows($hasilkd);
...
else if (isset($_GET['simpankd']))
	{
		...
		{	
			$querysimpankd="UPDATE kd_ket_bing SET kode_kd='$kkd[$k]', kd='$kd[$k]' WHERE nomor='$nomor[$k]'";
	...
	{ 	
		...
		for ($i=0; $i<$baris; $i++)
		{
			...
			{	
				...
				$querykd="SELECT * FROM kd_ket_bing WHERE nomor='$nomor'"; $hasilkd=mysql_query($querykd); $datakd = mysql_fetch_array($hasilkd);
				$himpunankd[$h] = array ($ratakd[$h] , $datakd[2]);
				//echo $himpunankd[$h][0]." ".$himpunankd[$h][1]."</br>";
				
				if($datas[4]==1)
				{
					$querysimpan = "UPDATE nilai_ket_bing_ganjil SET praktek_kd$indekskd ='".$praktekkd[$h][$i]."', produk_kd$indekskd ='".$produkkd[$h][$i]."', proyek_kd$indekskd ='".$proyekkd[$h][$i]."'
				...
				{
					$querysimpan = "UPDATE nilai_ket_bing_genap SET praktek_kd$indekskd ='".$praktekkd[$h][$i]."', produk_kd$indekskd ='".$produkkd[$h][$i]."', proyek_kd$indekskd ='".$proyekkd[$h][$i]."'
								WHERE no_absen='".$noabsen[$i]."'";
					$hasilsimpan=mysql_query($querysimpan) or die(mysql_error());
				}
			}
			...
			if($datas[4]==1)
			{
				$querysimpan = "UPDATE nilai_ket_bing_ganjil SET  nilai_100 ='$nilaiakhir[$i]', nilai_4 ='$nilai4[$i]', predikat ='$predikat[$i]', 
			...
			{
				$querysimpan = "UPDATE nilai_ket_bing_genap SET nilai_100 ='$nilaiakhir[$i]', nilai_4 ='$nilai4[$i]', predikat ='$predikat[$i]',
								deskripsi_positif= '$deskripsipositif[$i]' , deskripsi_negatif='$deskripsinegatif[$i]', keterangan ='$keterangan[$i]' WHERE no_absen='$noabsen[$i]'";
				$hasilsimpan=mysql_query($querysimpan) or die(mysql_error());
				$query="SELECT*FROM nilai_ket_bing_genap WHERE tahun_ajaran ='$datas[5]'"; $hasil=mysql_query($query) or die(mysql_error());
			}
	...
	$querykd="SELECT * FROM kd_ket_bing"; $hasilkd=mysql_query($querykd) or die(mysql_error());
	$bariskd=0; $i = 1;
		while ($datakd = mysql_fetch_array ($hasilkd))
		{	
			echo"<tr>";
