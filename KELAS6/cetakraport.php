<?php
	...
	if($datagk[4]==1)
	{
		$querypips="SELECT*FROM nilai_peng_ips_ganjil WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilpips=mysql_query($querypips) or die(mysql_error()); $datapipa=mysql_fetch_array ($hasilpipa);
		$querypipa="SELECT*FROM nilai_peng_ipa_ganjil WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilpipa=mysql_query($querypipa) or die(mysql_error()); $datapips=mysql_fetch_array ($hasilpips);
		...
		$worksheet->setCellValue('D225',$datapipa[12]);
		$worksheet->setCellValue('E225',$datapipa[14]);
		$worksheet->setCellValue('F225',$datapipa[15].'. '.$datapipa[16]);
		$worksheet->setCellValue('K225',$datakipa[12]);
		$worksheet->setCellValue('L225',$datakipa[14]);
		$worksheet->setCellValue('M225',$datakipa[15].'. '.$datakipa[16]);
		
		$worksheet->setCellValue('D233',$datapips[12]);
		$worksheet->setCellValue('E233',$datapips[14]);
		$worksheet->setCellValue('F233',$datapips[15].'. '.$datapips[16]);
		$worksheet->setCellValue('K233',$datakips[12]);
		$worksheet->setCellValue('L233',$datakips[14]);
		$worksheet->setCellValue('M233',$datakips[15].'. '.$datakips[16]);
		
		$worksheet->setCellValue('D242',$datapsbp[12]);
		$worksheet->setCellValue('E242',$datapsbp[14]);
		$worksheet->setCellValue('F242',$datapsbp[15].'. '.$datapsbp[16]);
		$worksheet->setCellValue('K242',$dataksbp[12]);
		$worksheet->setCellValue('L242',$dataksbp[14]);
		$worksheet->setCellValue('M242',$dataksbp[15].'. '.$dataksbp[16]);
		
		$worksheet->setCellValue('D251',$datappjok[12]);
		$worksheet->setCellValue('E251',$datappjok[14]);
		$worksheet->setCellValue('F251',$datappjok[15].'. '.$datappjok[16]);
		$worksheet->setCellValue('K251',$datakpjok[12]);
		$worksheet->setCellValue('L251',$datakpjok[14]);
		$worksheet->setCellValue('M251',$datakpjok[15].'. '.$datakpjok[16]);
		
		$worksheet->setCellValue('D260',$datapbd[12]);
		$worksheet->setCellValue('E260',$datapbd[14]);
		$worksheet->setCellValue('F260',$datapbd[15].'. '.$datapbd[16]);
		$worksheet->setCellValue('K260',$datakbd[12]);
		$worksheet->setCellValue('L260',$datakbd[14]);
		$worksheet->setCellValue('M260',$datakbd[15].'. '.$datakbd[16]);
		
		$worksheet->setCellValue('B277',$dataekskul[3]);
		$worksheet->setCellValue('B278',$dataekskul[4]);
		$worksheet->setCellValue('B279',$dataekskul[5]);
		$worksheet->setCellValue('B280',$dataekskul[6]);
		$worksheet->setCellValue('B281',$dataekskul[7]);
		
		$worksheet->setCellValue('B286',$dataekskul[9]);
		
		if($datagk[4]==1)
		{
			$worksheet->setCellValue('F295',$datakesehatan[3]);
			$worksheet->setCellValue('F296',$datakesehatan[5]);
		}
		else if($datagk[4]==2)
		{
			$worksheet->setCellValue('J295',$datakesehatan[4]);
			$worksheet->setCellValue('J296',$datakesehatan[6]);
		}
		
		$worksheet->setCellValue('F302',$datakesehatan[7]);
		$worksheet->setCellValue('F303',$datakesehatan[8]);
		$worksheet->setCellValue('F304',$datakesehatan[9]);
		$worksheet->setCellValue('F305',$datakesehatan[10]);
		
		$worksheet->setCellValue('N310',$datadaftarhadir[3]);
		$worksheet->setCellValue('N311',$datadaftarhadir[4]);
		$worksheet->setCellValue('N312',$datadaftarhadir[5]);
		
		// redirect output to browser
		header('Content-Type: application/vnd.openxmlformatsofficedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="raport_kelas6_ganjil.xls"');
	...
	{	//NILAI GANJIL
		$querypips1="SELECT*FROM nilai_peng_ips_ganjil WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilpips1=mysql_query($querypips1) or die(mysql_error()); $datapips1=mysql_fetch_array ($hasilpips1);
		$querypipa1="SELECT*FROM nilai_peng_ipa_ganjil WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilpipa1=mysql_query($querypipa1) or die(mysql_error()); $datapipa1=mysql_fetch_array ($hasilpipa1);
		...
		$querykips1="SELECT*FROM nilai_ket_ips_ganjil WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilkips1=mysql_query($querykips1) or die(mysql_error()); $datakipa1=mysql_fetch_array ($hasilkipa1);
		$querykipa1="SELECT*FROM nilai_ket_ipa_ganjil WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilkipa1=mysql_query($querykipa1) or die(mysql_error()); $datakips1=mysql_fetch_array ($hasilkips1);
		...
		//NILAI GENAP
		$querypips="SELECT*FROM nilai_peng_ips_genap WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilpips=mysql_query($querypips) or die(mysql_error()); $datapips=mysql_fetch_array ($hasilpips);
		$querypipa="SELECT*FROM nilai_peng_ipa_genap WHERE tahun_ajaran ='$datagk[5]' AND no_absen='$noabsen'"; $hasilpipa=mysql_query($querypipa) or die(mysql_error()); $datapipa=mysql_fetch_array ($hasilpipa);
		...
		$totalpeng1 = ($datapagama1[12]+$datappkn1[12]+$datapbi1[12]+$datapmtk1[12]+$datapsbp1[12]+$datappjok1[12]+$datapbd1[12]+$datapipa1[12]+$datapips1[12])/9;
		$totalket1 = ($datakagama1[12]+$datakpkn1[12]+$datakbi1[12]+$datakmtk1[12]+$dataksbp1[12]+$datakpjok1[12]+$datakbd1[12]+$datakipa1[12]+$datakips1[12])/9;
		
		$totalpeng = ($datapagama[12]+$datappkn[12]+$datapbi[12]+$datapmtk[12]+$datapsbp[12]+$datappjok[12]+$datapbd[12]+$datapipa[12]+$datapips[12])/9;
		$totalket = ($datakagama[12]+$datakpkn[12]+$datakbi[12]+$datakmtk[12]+$dataksbp[12]+$datakpjok[12]+$datakbd[12]+$datakipa[12]+$datakips[12])/9;
		...
		{
			$worksheet->setCellValue('G313','5');
			$worksheet->setCellValue('H313','( Lima )');
			//echo"LULUSSSS</br>";
		}
		else if (($totalpeng >= $datakkm[0]) and ($totalket >= $datakkm[0]) and ($datadaftarhadir[5]>>3)
			and ($totalpeng1 >= $datakkm[0]) and ($totalket1 >= $datakkm[0]) and ($datadaftarhadir1[5]>>3))
		{
			$worksheet->setCellValue('G314','4');
			$worksheet->setCellValue('H314','( Empat )');
			//echo"TIDAK LULUSS";
		}
		else if (($totalpeng << $datakkm[0]) and ($totalket << $datakkm[0]) and ($datadaftarhadir[5]<=3)
			and ($totalpeng1 << $datakkm[0]) and ($totalket1 << $datakkm[0]) and ($datadaftarhadir1[5]<=3))
		...
		// redirect output to browser
		header('Content-Type: application/vnd.openxmlformatsofficedocument.spreadsheetml.sheet');
		header('Content-Disposition: attachment;filename="raport_kelas6_genap.xls"');
		header('Cache-Control: max-age=0');
		$objWriter = new PHPExcel_Writer_Excel5($objPHPExcel); // 2007 untuk ex2007, 5 untuk versi excel seterusnya
		$objWriter->save('php://output');
	}
